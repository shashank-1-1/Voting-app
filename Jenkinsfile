                    ...
                    withCredentials([file(credentialsId: 'kubeconfig', variable: 'KUBECONFIG_FILE')]) {
                        sh '''
                            ...
                            if [ -z "$SERVICE_IP" ]; then
                                echo "Service IP not available, using ngrok instead"
                                ...
                                if [ $? -eq 0 ]; then
                                    echo "Pod is now Running, starting ngrok tunnel..."
                                    nohup ngrok http 5000 &
                                    sleep 10
                                    echo "ngrok tunnel started, you can access your app externally!"
                                fi
                            else
